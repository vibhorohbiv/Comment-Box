...
const onSubmit = () => {
    ...
    callBackData['date'] = new Date().toDateString("DD/mm/yyyy").split(" ").slice(1).join(" ")
    callBackData['time'] = [new Date().getHours(),new Date().getMinutes()].join(":") 
      ...
      <div className="container">
        <div className="chat-box">
          <div className="col-12">
            <MessageDisplay  message={message} replyCallBack={replyCallBack} deleteReply={deleteReply}/>
          </div>
          <div className="col-12">
            <MessageBox setDetails={setDetails} onSubmit={onSubmit} messageDetails={messageDetails} reply={reply}/>
          </div>    
        </div>
      </div>
    </div>
